...
$._PPP_ = {
    ...
    fillMogrt : function (items) {
	  var proj = app.project;
	  	if (proj){
	  	  for (var i = 0; i < items.length; i++) {
            alert(items[i].toString());
          }
		}  
    },
    ...
    buildPath: function(currentFolder, additionalDirectory, fileName, parentLevels) {
        var folder = new Folder(currentFolder);
        for (var i = 0; i < parentLevels; i++) {
            folder = folder.parent;
        }
        var path = folder.fsName + "\\" + additionalDirectory + "\\" + fileName;
        return path;
    },
    ...
    getBinItemParams: function() {
        var param = [];
        var seq = app.project.activeSequence;
        if (seq) {
            var sel = seq.getSelection();
            if (sel && (sel !== "Connection to object lost") && sel.length === 1) {
                if (sel[0].name !== 'anonymous') {
                    if (sel[0].isMGT()) {
                        var mogrt = sel[0].getMGTComponent();
                        for (var i = 0; i < mogrt.properties.numItems; i++) {
                            values.push(mogrt.properties[i].displayName.toString());
                        }
                    }
                }
                return values;
    ...
    getTimelineItemParams: function() {
        var param = '';
        var seq = app.project.activeSequence;
        if (seq) {
            var sel = seq.getSelection();
    ...
    getMogrtVals: function() {
        ...
        if (seq) {
            ...
            if (sel && (sel !== "Connection to object lost") && sel.length === 1) {
                if (sel[0].name !== 'anonymous') {
                    if (sel[0].isMGT()) {
                        var path = sel[0].projectItem.getMediaPath();
                        var mogrt = sel[0].getMGTComponent();
                        for (var i = 0; i < mogrt.properties.numItems; i++) {
                            values.push(mogrt.properties[i].displayName.toString());
                        }
        ...
        if (seq) {
            ...
            if (sel && (sel !== "Connection to object lost") && sel.length === 1) {
                if (sel[0].name !== 'anonymous') {
                    if (sel[0].isMGT()) {
                        var mogrt = sel[0].getMGTComponent();
                        for (var i = 0; i < item["mogrtPaths"].length; i++) {
                            mogrt.properties[item["mogrtPaths"][i]].setValue(item["values"][i], true);
                        }
                    }
                }
            }
    ...
    myActiveSequenceSelectionName: function() {
        var seq = app.project.activeSequence;
        if (seq) {
            var sel = seq.getSelection();
            if (sel && (sel !== "Connection to object lost") && sel.length === 1) {
    ...
    myActiveSequenceSelectionChangedFxn: function() {
        ...
        if (seq) {
            ...
            if (sel && (sel !== "Connection to object lost") && sel.length === 1) {
                if (sel[0].name !== 'anonymous') {
                    if (sel[0].isMGT()) {
                        $._PPP_.dispatchEventCEP("CurrentSeqSel", sel[0].name);
                    }
                }
            } else {
                $._PPP_.dispatchEventCEP("CurrentSeqSel", '');
            }
        } else {
            $._PPP_.dispatchEventCEP("CurrentSeqSel", '');
        }
    ...
    createSubSequence: function() {
        ...
        */
        ...
        if (activeSequence) {
            ...
            for (var a = 0;
                (a < activeSequence.videoTracks.numTracks && targetTrackFound === false); a++) {
...
// alert(scriptPath);
// alert($._PPP_.buildPath($.fileName, 'logo items', 'pink.png', 0));
//$._PPP_.importFile($._PPP_.buildPath($.fileName, 'logo items', 'pink.png', 2));